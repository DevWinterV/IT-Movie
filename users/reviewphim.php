...
<head>
<?php
    $movieURL = "index.php?page_layout=reviewphim&id=$id"; // Thay đổi URL chi tiết phim tương ứng
    $_SESSION['redirect'] = $movieURL;
...
<body>
    <div class="tenphim">
        <?php
            echo "<h2   style='color: white ' >Bạn đang xem phim ".$row['title']." - ".$row['language']." - Full HD - IT Phim</title></h2>";
        ?>
    </div>
    <div class="videophim">
        <iframe width="900" height="500" src="https://www.youtube.com/embed/<?php echo "".$row['link2']."";?>" frameborder="0" allowfullscreen></iframe>
    </div>
    <div class="chitiet">
     <?php
  ...
  <div class='left-column'>
    ...
    if($rowtheloai != null){
        ...
        while ($rowmoviesNew = $ListMovieNew->fetch_array(MYSQLI_ASSOC)) {
            ...
            echo "<a href='chitietphim.php?id=" . $rowmoviesNew["movie_id"] . "' class ='card-link'>";
...
<div class='pagination'>
    ...
    if ($currentPage > 1) {
        echo "<a href='?page_layout=reviewphim&id=$ID&page=". ($currentPage - 1) . "'><img src='anhnen/previous.png' alt='Back' style='width: 20px; height: 20px;'></a> ";
    }
    // Hiển thị các trang
    ...
    if ($currentPage < $totalPages) {
        echo "<a href='?page_layout=reviewphim&id=$ID&page=".($currentPage + 1) ."'><img src='anhnen/next.png' alt='Next' style='width: 20px; height: 20px;'></a> ";
    }
    ?>
</div>
<script>
...
function changeBackgroundImage() {
    ...
    // Kiểm tra nếu đã đạt đủ 2 phút (120 giây)
    if (elapsedTime >= 120) {
